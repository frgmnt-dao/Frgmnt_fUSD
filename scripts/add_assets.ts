...
async function main() {
  const ASSET_HANDLER_PROXY =
    "0xB401f781C66c2A7f73b123E2E142F2Ad860fD5F0";
  ...
  const assetHandler = await ethers.getContractAt(
    "AssetHandler",
    ASSET_HANDLER_PROXY,
    signer
  );
  ...
  for (const asset of assets) {
    ...
    const tx1 = await assetHandler.addAsset(
      asset.address,
      asset.type,
      asset.feed,
    ...
    const tx2 = await assetHandler.setChainlinkTimeout(
      asset.address,
      ONE_DAY,
      { ...overrides, nonce: nonce++ }
    );

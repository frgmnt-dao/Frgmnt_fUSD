...
async function main() {
  ...
  // Load contracts
  // --------------------------------------------------
  const pm = await ethers.getContractAt('PoolManagerLogic', POOL_MANAGER_PROXY);

  const pl = await ethers.getContractAt('PoolLogic', POOL_LOGIC_PROXY);
  ...
  const ok1 = pm_poolLogic.toLowerCase() === POOL_LOGIC_PROXY.toLowerCase();

  const ok2 = pl_poolManager.toLowerCase() === POOL_MANAGER_PROXY.toLowerCase();

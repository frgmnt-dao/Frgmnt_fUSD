...
- `CashWithdrawImmediateProRata` and `AttestedWithdrawPlanExecuted` are now emitted from inside `WithdrawalPlanLib.executeWithdrawalPlan()` rather than from `PoolLogic` — a delegatecall preserves the caller's address for the EVM's `LOG` opcode, so this doesn't change what a consumer of `PoolLogic`'s own event log sees, but it was necessary to keep this fitting in `PoolLogic`'s EIP-170 budget at all (see the design doc's "Bytecode size budget" section (4.1) for the exact headroom numbers before and after this change). `PoolLogic` still declares both events for ABI completeness; it no longer contains the code to emit them from this path.

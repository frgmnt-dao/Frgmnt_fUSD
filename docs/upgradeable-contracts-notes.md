...
- **Custody is contradicted across the repo.** `upgrade_core_contracts.ts` records the AssetHandler and PoolManagerLogic `ProxyAdmin`s and `factoryOwner` as the `GOVERNANCE_SAFE` EOA; `upgrade_attested_withdrawal.ts` records them as the DAO Safe. Both scripts now read the owners on-chain at run time and refuse to proceed on a mismatch (`ALLOW_CUSTODY_MISMATCH=1` overrides the core script's check). If `factoryOwner` is still a single key, the latched emergency stop, `setMaxSurchargeBps` and the rotation-delay floor depend on that one key.
...
```text
npx hardhat compile: passed
npx hardhat test test/AttestedWithdrawal.test.ts test/SelectiveGuards.test.ts test/SelectiveGuardDeploy.test.ts: 144 passing
npm run test: 1226 passing (1 opt-in test pending)

...
- **`GOVERNANCE_SAFE` is intended to be a Gnosis Safe multisig** (per `Timelock.sol`'s deployment comment), but **on the live USD deployment it is confirmed to actually be a plain EOA** (`0xafb9B883637f72767ADf7193Bb3B8e59C02Ea05d` has no contract code on-chain, checked directly via `eth_getCode` against Base mainnet). A single private key currently controls `Governance.owner`, `AssetHandler.owner`, `PoolManagerLogic.factoryOwner`, and `TokenLogic.DEFAULT_ADMIN_ROLE`. By contrast, `PoolLogic`'s `ProxyAdmin` has already been migrated (via the pre-existing `scripts/transferRoles_poolLogic.ts`) to a genuine 3-of-4 Gnosis Safe at a *different* address (`0x74aF72D91D5FB263fBa09Ed43aD1C1ea079058B3`, confirmed via `getOwners()`/`getThreshold()`) — so today, only one of the five core proxies is actually multisig-protected. Migrating the other four to a real multisig (this DAO Safe or a new one) is a deliberate, separate decision for the team to make; not yet done.

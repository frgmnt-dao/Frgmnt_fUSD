...
| Fee-on-transfer settlement asset on the queued-withdrawal claim side: `claimCashWithdraw()` trusted the reserved nominal `assetAmount` to be exactly what moved on both the pool's own balance and the claimant's, but a sender-fee/burn-on-transfer asset can drain more than that from the pool's own balance, and a recipient-fee asset can deliver less to the claimant (FNA-23 follow-up) | `FundCalculationLibrary.claimCashWithdrawTransfer()` (extracted purely to keep `PoolLogic`'s own bytecode under the EIP-170 limit) measures the actual balance delta on both sides. `reservedAssetBalance[asset]` is now decremented by what genuinely left the pool's own balance, not the nominal amount — a sender-fee token draining more than expected would otherwise leave `reservedAssetBalance` overstated relative to the real on-chain balance, tripping the FNA-03 invariant (`balanceOf >= reserved`) on every later guarded transaction and finalization for as long as that overstatement persisted. `fusdNetForAsset` is still burned in full regardless of what the claimant actually received: the reserved value genuinely left the pool's balance either way, whether it landed with the claimant or was consumed by the asset's own fee mechanism en route — refunding the shortfall as unburned fUSD would create a claim no longer backed by the (already-departed) reserved asset. The `CashWithdrawClaimed` event now reports what the claimant actually received, not the nominal reserved amount, for accurate off-chain accounting |

...
| `isAttestedWithdrawEnabled` (bool)                                   | `PoolLogic` (new state) | Instantly togglable via `setAttestedWithdrawEnabled(bool)` by the manager; the `factoryOwner` may call it too but **only to switch it off**; **independent** of `isImmediateWithdrawEnabled` |

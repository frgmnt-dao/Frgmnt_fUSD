...
**The size limit forced the relocation.** At the baseline, `PoolLogic` had only **129 bytes** of EIP-170 headroom. The new entry point, its governance functions, its events and errors and its storage are far larger than 129 bytes (the relocation freed roughly 3,400 bytes, and the feature's own additions to `PoolLogic` then used nearly all of it: the net change in deployed size is only −39 bytes, from 24,447 to 24,408), so it cannot be added to `PoolLogic` without first freeing bytecode. The only structural options are to move existing code out of `PoolLogic`, or not to add the feature.
...
3. Attester signs a WithdrawalPlan
   ...
   user, with a short deadline.
                                        ...
                                        10. Measure total fund value
                                            ...
                                            within [target * (1 -
                                            minValueOutBps), target +
                                            DUST_TOLERANCE], where
                                            target = fairFusd minus the
                                            surcharge; value measured on
                                            the uncapped complete NAV.
                                                │
                                                ▼
...
struct WithdrawalPlan {
    ...
    uint256 minValueOutBps;              // attester-chosen ACCEPTABLE under-delivery tolerance, in bps of
                                          ...
                                          // 99.80% of `target` (fairFusd minus the surcharge) for this plan. This is an ENFORCED
    ...
    uint256 maxAcceptableSurchargeBps;    // attester's ceiling on the live pool-usage surcharge for this
                                          // plan (see Surcharge); 0 = reject any nonzero surcharge
}
...
3. **Freshness & replay checks.** `require(block.timestamp <= plan.deadline)` (`PlanDeadlineExpired`); `require(!consumedPlanNonce[plan.user][plan.nonce])` (`PlanNonceAlreadyUsed`) — the library reads the nonce's current value itself through a self-call to the public `consumedPlanNonce` getter (`_loadPlanInput`); the actual `consumedPlanNonce[...] = true` write happens back in `PoolLogic`, **after** the library call returns successfully, not immediately on this check — see the note under [High-Level Flow](#high-level-flow) on why that's safe despite differing from this document's original intent.
...
8. **Measure fund value before, compute `fairFusd`.** `(, totalClaims, completeBefore) = computeImmediateWithdrawPortion(pool, netFusd, 1)` (the `1` only avoids the function's zero early-return; only `completeFundValue` is read), then `fairFusd = applyClaimsHaircut(netFusd, completeBefore, totalClaims)` — computed before the allocations loop so `completeFundValue` reflects pre-withdrawal state for the accounting step later. `completeBefore` is the **uncapped**, net-realizable, deficit-adjusted, reserved-excluding NAV, and it is the yardstick for value conservation (see [Value Conservation](#value-conservation-the-core-safety-invariant)) and the surcharge denominator. `fairFusd == 0` reverts `WithdrawAmountTooSmall` (extreme insolvency or an empty pool, mirroring the pro-rata path's zero-portion check). The earlier `fairFusd > valueBefore` fail-fast was removed: it compared against the liquidity-capped NAV, which is no longer used.
9. **Per-asset withdrawal.** For each entry in `allocations`, validated as it's processed rather than in a separate upfront pass: asset-membership (`isSupportedAsset`) and duplicate checks, then the guard binding (`alloc.guard` must equal the current `getAssetGuard(asset)`, else `GuardMismatch`); if `positionIds` is non-empty the position-level path applies (see [Position-Level Selection](#position-level-selection-asset-guard-positions)); otherwise, if `useFixedAmount`, first `require(guard.getBalance(pool, asset) > 0)` (`ZeroAssetBalance`) then compute `portion = min(fixedAmount * 1e18 / balance, 1e18)`; otherwise use `allocations[i].portion` directly, rejecting outright (`InvalidPortion`) if it exceeds `1e18` (a bound added during review — see the [Data Structures](#data-structures) section). Look up the matching `ComplexAsset` from `complexAssetsData` by address, and process the withdrawal for that asset via the same per-asset guard-dispatch logic the pro-rata path now also calls (`WithdrawalPlanLib.withdrawProcessing`).
10. **Value-conservation check.** Compare the uncapped complete NAV measured in step 8 (after the burn) with the same reading after step 9 completes, against `target = fairFusd - surchargeAmount` — **not** the raw `netFusd`; see the correction below:
...
**Correction (final review round): value is measured on the uncapped NAV, not the liquidity-capped one.** An earlier revision measured `valueBefore`/`valueAfter` with `computeWithdrawableFundValue` (the liquidity-capped NAV the pro-rata path sizes its portion against) and added a `fairFusd > valueBefore` fail-fast for the temporary-liquidity-gap case. That yardstick is only sound when every guard applies one uniform portion. For a guard whose liquidity ceiling is a MINIMUM across several positions (Morpho Blue markets, Aave V3 reserves), the capped balance does not move linearly when only some positions are drawn, so the measured outflow can differ from the real outflow by the inverse of the binding liquidity ratio: legitimate plans could revert, and — worse — the upper bound could be weakened, letting a plan extract several times `target` while the capped reading still looked like `target`. The fix measures real outflow on the uncapped complete NAV (`completeFundValue`, oracle-priced position value) before and after the loop; the same figure feeds `computeAccountedAssetsReduction`, which already expects it. The liquidity-gap case is now caught by the lower bound (`ValueConservationViolated`) after the loop instead of failing fast; it is a gas/clarity difference only, not a fund-safety one, and the loop reverts atomically. This change is also the prerequisite for letting a plan choose individual positions inside a guard.
...
| `isAttestedWithdrawEnabled` (bool)                                   | `PoolLogic` (new state) | Instantly togglable via `setAttestedWithdrawEnabled(bool)` by the manager; the `factoryOwner` may call it too: `false` switches it off **and latches the stop** (`attestedWithdrawOwnerStopped`, which the manager cannot override), `true` only lifts the latch and never itself enables the feature; **independent** of `isImmediateWithdrawEnabled` |
...
- `setAttestedWithdrawEnabled(false)` — **instant**, callable by the manager **or the `factoryOwner`** (when the `factoryOwner` calls it, the stop is **latched**: the manager cannot switch the feature back on until the `factoryOwner` lifts it, and the `factoryOwner` lifting it does not itself enable anything). This is a pure capability _removal_; there is never a reason to delay it, and it must be available immediately if the backend key is suspected compromised. This is the incident-response kill switch. The `factoryOwner` is included deliberately: the manager otherwise controls the attester rotation and the volume cap, so a compromised or colluding manager could appoint its own attester and lift the circuit breaker with nobody able to stop it. Switching the feature off also clears any pending attester proposal, so a malicious proposal still inside its rotation delay cannot be activated afterwards. Without the latch the stop would be defeatable: the manager could re-enable at once, or re-propose an attester, wait out the 24h delay while disabled, and enable again, so the `factoryOwner` would have to win a whack-a-mole every rotation period. With it, re-enabling requires the `factoryOwner` to lift the stop first (after reviewing the attester) and then the manager to enable. A manager-initiated disable is not latched. On mainnet the `factoryOwner` is a Safe, so the stop needs a Safe quorum rather than a single key; the manager's own disable remains the instant, no-coordination lever. Edge case: if the manager address is later set equal to the `factoryOwner` while a stop is latched, that address acts as the manager and cannot lift it until the manager changes.
...
**The mechanism.** Computed inside `WithdrawalPlanLib.executeWithdrawalPlan()`, immediately after `completeBefore` and the circuit breaker's updated volume state are known — deliberately before the per-asset allocations loop runs (see below for why):

```solidity
pressure = min(decayedAccumulatedVolume(including this withdrawal) * 1e18 / completeBefore, 1e18)
...
- **Minimum plan size (`MIN_PLAN_NET_FUSD` = $0.01 net).** `DUST_TOLERANCE` is an absolute 1e15 wei, and the volume breaker meters `netFusd`. A plan burning a few wei of fUSD could therefore release up to $0.001 of real value per transaction while barely registering in the breaker — a compromised-attester-only path (each plan needs a fresh nonce and a signature), uneconomic on mainnet but weaker on a cheap L2. Plans below the floor revert `WithdrawAmountTooSmall`, so the burn is always at least 10x the most the tolerance can release. Smaller redemptions use the pro-rata path.
- **Accepted, documented residuals (all inherited, none new trust).**
  - The deficit reading (`_guardDeficit`, shared validated code) fails open if a deficit-reporting guard's `getDeficit` succeeds before the loop and fails after it, which would understate `valueDelta` by the deficit. It is the same behavior as the pro-rata path and cannot be forced under EIP-150 gas rules; not changed, because the function is validated code.
  - The library `safeTransfer`s whatever `(withdrawAsset, withdrawAmount)` a guard returns. A governed guard already controls arbitrary calls executed as the pool via its returned transactions, which is strictly stronger, so an allow-list here would add no protection. The two selective guards return `(address(0), 0, txs)`.
  - Guard-executed legs (Spoke, Morpho) are absent from `outAssets` and the `CashWithdrawImmediateProRata` event, as on the whole-asset path; indexers must reconstruct them.
  - Any unpriced or unvaluable position anywhere makes the NAV incomplete and reverts a plan even for a healthy, unrelated selection (`IncompleteNAV`): a liveness limit in the safe direction.
  - A Morpho subset call still reads every tracked market through the validated collectors, so an unselected market whose IRM or oracle reverts can revert the call, and gas scales with tracked markets. Only the liquidity throttle is decoupled from unselected markets. Donating supply shares to a near-fully-utilised selected market can throttle its ceiling toward zero (the base guard's accepted residual); the attester can drop that market from the plan.
...
**As implemented:** `test/AttestedWithdrawal.test.ts` carries dedicated tests (71 at the time of writing, plus 23 in `test/SelectiveGuards.test.ts`) covering every item above, plus several added during post-implementation review that weren't anticipated in the original plan: a haircut-parity test mirroring the existing FNA-05 underwater-pool fixture exactly (confirming an attested plan now gets haircut the same as the pro-rata path instead of paying out at par); a dedicated "temporary liquidity gap" test (using a guard's own withdrawable-balance cap, independent of the underwater-pool case) asserting `ValueConservationViolated` on the uncapped-NAV measurement; and two tests added specifically because `complexAssetsData` had zero direct coverage through this entry point — one confirming `slippageTolerance` is actually enforced on regular guard dispatch, one confirming full `IComplexAssetGuard` processing (non-empty `withdrawData`) dispatches and propagates guard reverts correctly. The full repository test suite passes with this feature included.

...
**The withheld value is retained by shrinking the withdrawal's `accountedAssets` reduction, not by crediting it separately.** `PoolLogic`'s existing accounting already computes a `reduction` to apply to `accountedAssets` for any withdrawal (via `FundCalculationLibrary.computeAccountedAssetsReduction`); the surcharge simply subtracts `surchargeAmount` from that `reduction` before it's applied (floored at zero), so the user still burns their full entitlement of fUSD, but `accountedAssets` ends up higher than an equivalent surcharge-free withdrawal would have left it. This detail matters more than it looks: `accountedAssets` is the baseline the fund's yield-accrual logic measures against to compute the manager's performance fee, and that baseline only ever rises through ordinary yield recognition. Crediting the surcharge any other way — as a plain, separate increase to fund value — would let the very next accrual event read it as ordinary yield and mint the manager a performance-fee cut of it, silently contradicting the entire point of retaining this value for remaining stakers rather than the person operating the fund. Folding it into the existing reduction avoids that leak entirely, at the cost of zero extra storage writes.

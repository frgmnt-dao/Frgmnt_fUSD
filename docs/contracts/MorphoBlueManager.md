...
> **CertiK FNA-52 follow-up**: `morpho` used to be a caller-supplied parameter to this function. Since `pruneTrackedMarket` is deliberately permissionless, anyone could pass a stub contract whose `position()` always returns an empty `Position`, untracking a delisted market that still holds a live position on the _real_ Morpho Blue — silently restoring the exact NAV/withdrawal-safety gap FNA-52 had just closed. `morpho` is now `immutable`, fixed at deploy time via the constructor (which reverts with `MorphoZero()` on a zero address) — there is no longer any address for a caller to spoof.

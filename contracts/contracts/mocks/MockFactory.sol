...
contract MockFactory {
  ...
  // ----------------- setters for tests -----------------
  function setOwner(address _new) external { owner_ = _new; }
  function setMaximumSupportedAssetCount(uint256 n) external { maximumSupportedAssetCount = n; }
  function setValidAsset(address a, bool v) external { validAsset[a] = v; }
  function setAssetPrice(address a, uint256 p) external { assetPrice[a] = p; }
  function setAssetType(address a, uint16 t) external { assetType[a] = t; }
  function setMaxFees(uint256 p, uint256 m, uint256 eIn, uint256 eOut, uint256 denom) external {
    maxPerf = p; maxMgmt = m; maxEntry = eIn; maxExit = eOut; feeDenominator = denom;
  }
  function setPerfChange(uint256 maxChange, uint256 delaySecs) external {
    maxPerfChange = maxChange; perfChangeDelay = delaySecs;
  }
  function setAssetGuard(address g) external { assetGuard = g; }
  function setIsPool(address a, bool v) external { isPoolAddr[a] = v; }
  ...
  // IHasAssetInfo
  function isValidAsset(address a) external view returns (bool) { return validAsset[a]; }
  function getMaximumSupportedAssetCount() external view returns (uint256) { return maximumSupportedAssetCount; }
  function getAssetPrice(address a) external view returns (uint256) {
    uint256 p = assetPrice[a];
    return p == 0 ? 1e18 : p; // default 1 USD
  }
  function getAssetType(address a) external view returns (uint16) { return assetType[a]; }
  ...
  // IHasGuardInfo
  function getAssetGuard(address) external view returns (address) { return assetGuard; }

  // IPoolFactory bits PoolManagerLogic actually uses
  function isPool(address a) external view returns (bool) { return isPoolAddr[a]; }
  function emitPoolManagerEvent() external { emit PoolManagerEventEmitted(); }
}

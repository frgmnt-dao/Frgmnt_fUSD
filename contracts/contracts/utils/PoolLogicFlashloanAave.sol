...
abstract contract PoolLogicFlashloanAave {
    ...
    function _executeAaveFlashloan(
        ...
        uint256 withdrawAssetBalanceBefore = IERC20(asset).balanceOf(address(this));

        IAssetGuard.MultiTransaction[] memory transactions = IAaveLendingPoolAssetGuard(guard)
            .flashloanProcessing(address(this), asset, amount, premium, params);

        bool success;
        bytes memory returndata;

        for (uint256 i = 0; i < transactions.length; ++i) {
            (success, returndata) = transactions[i].to.call(transactions[i].txData);
        ...
        require(
            withdrawAssetBalanceBefore + premium <= IERC20(asset).balanceOf(address(this)),
            "high slippage"
        );
    ...
    function _validateAaveGuard(address guard) internal view returns (address) {
        (bool ok, bytes memory data) = guard.staticcall(
            abi.encodeWithSelector(IAaveLendingPoolAssetGuard.aaveLendingPool.selector)
        );

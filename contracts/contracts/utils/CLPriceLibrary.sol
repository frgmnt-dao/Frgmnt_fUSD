...
/// @title CLPriceLibrary
/// @notice Chainlink/Uniswap V3 price helper library used in the Frgmnt / dHEDGE system.
library CLPriceLibrary {
  ...
  function isSqrtPriceDeviationInRange(
    ...
    uint256 threshold = fee >= MIN_THRESHOLD ? uint256(fee) * 150 / 100 : MIN_THRESHOLD;

    uint256 fair = uint256(fairSqrtPriceX96);
    uint256 current = uint256(sqrtPriceX96);
    uint256 delta = fair * threshold / 1_000_000;

    return (current < fair + delta && fair < current + delta);
  }
  ...
  function calculateSqrtPrice(
    ...
    // price = token1Amount / token0Amount =  (token0Price * token1Decimals) / (token1Price * token0Decimals)
    // sqrtPriceX96 = sqrt(price) * 2^96

    // adjust for the part: (token0Price * token1Decimals) / token1Price
    // ideal for token1Decimals being 18
    if (token1Decimals < 18) {
      token1DecimalsConversion = ((18 - token1Decimals) / 2 + 1);
      priceRatio = token0Price * (10 ** (token1Decimals + token1DecimalsConversion * 2)) / token1Price;
    } else {
      priceRatio = token0Price * (10 ** token1Decimals) / token1Price;
    }
    ...
    // (priceRatio << 192) is done on uint256 in 0.8, safe with the above bound
    uint256 radicand = (priceRatio << 192) / (10 ** token0Decimals);
    sqrtPriceX96 = uint160(DhedgeMath.sqrt(radicand));

    if (overflowProtection) {
      // increase 5 decimals (revert adjustment)
      sqrtPriceX96 = uint160(uint256(sqrtPriceX96) * (10 ** 5));
    }

    // reverse the init token1Decimals adjustment
    sqrtPriceX96 = uint160(uint256(sqrtPriceX96) / (10 ** (token1DecimalsConversion)));
  }
}

...
contract SlippageAccumulator is Ownable {
	...
	function updateSlippageImpact(
		...
		if (IHasSupportedAsset(poolManagerLogic).isSupportedAsset(swapData.srcAsset)) {
			...
			if (dstValue < srcValue) {
				...
				managerData[poolManagerLogic].lastTradeTimestamp = uint256(block.timestamp).toUint64();
	...
	function getCumulativeSlippageImpact(address poolManagerLogic) public view returns (uint128 cumulativeSlippage) {
		...
		uint256 timeSinceLastTrade = block.timestamp - uint256(managerSlippageData.lastTradeTimestamp);
		...
		uint256 adjusted = (uint256(managerSlippageData.accumulatedSlippage) * effectiveTime) / uint256(decayTime);

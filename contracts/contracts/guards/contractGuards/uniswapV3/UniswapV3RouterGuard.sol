  ...
  function txGuard(
    ...
    if (method == IV3SwapRouter.exactInput.selector) {
      ...
      IV3SwapRouter.ExactInputSingleParams memory params =
        abi.decode(getParams(data), (IV3SwapRouter.ExactInputSingleParams));

      require(poolManagerLogicAssets.isSupportedAsset(params.tokenOut), "Frgmnt: unsupported destination asset");
      require(poolLogic == params.recipient, "Frgmnt: recipient is not pool");
      ...
      IV3SwapRouter.ExactOutputSingleParams memory params =
        abi.decode(getParams(data), (IV3SwapRouter.ExactOutputSingleParams));

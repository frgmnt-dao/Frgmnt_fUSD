...
contract UniswapV3NonfungiblePositionGuard is TxDataUtils, ITxTrackingGuard {
    ...
    returns (uint16 txType, bool)
  {
    ...
    if (method == INonfungiblePositionManager.mint.selector) {
      INonfungiblePositionManager.MintParams memory param =
        abi.decode(getParams(data), (INonfungiblePositionManager.MintParams));
      ...
      INonfungiblePositionManager.IncreaseLiquidityParams memory param =
        abi.decode(getParams(data), (INonfungiblePositionManager.IncreaseLiquidityParams));
      ...
      INonfungiblePositionManager.DecreaseLiquidityParams memory param =
        abi.decode(getParams(data), (INonfungiblePositionManager.DecreaseLiquidityParams));
      ...
      INonfungiblePositionManager.CollectParams memory param =
        abi.decode(getParams(data), (INonfungiblePositionManager.CollectParams));

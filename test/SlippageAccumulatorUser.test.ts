...
describe("SlippageAccumulatorUser", () => {
  ...
  beforeEach(async () => {
    ...
    const MockAccumulator = await ethers.getContractFactory(
      "MockSlippageAccumulatorCore"
    );
    ...
    const MockUser = await ethers.getContractFactory(
      "MockSlippageAccumulatorUser"
    );
    slippageUser = await MockUser.deploy(
      await mockAccumulator.getAddress()
    );
    await slippageUser.waitForDeployment();

    // 3) Deploy your existing MockPoolManagerLogic
    const MockPoolManagerLogic = await ethers.getContractFactory(
  ...
  it("reverts when accumulator address is zero", async () => {
    ...
    await expect(
      MockUser.deploy(ethers.ZeroAddress)
  ...
  it("calls accumulator and clears intermediateSwapData when called by poolLogic", async () => {
    ...
    await expect(
      slippageUser
        ...
        .afterTxGuard(poolManagerAddr, routerAddress, "0x")
    )
  ...
  it("returns ERC20 balance for token address", async () => {
    ...
    const bal = await slippageUser.exposedGetBalance(

...
describe("Managed", () => {
  ...
  describe("changeManager", () => {
    it("allows current manager to change manager and emits event", async () => {
      const { managed, accounts } = await loadFixture(setupFixture);
      await expect(
        managed.connect(accounts.manager).changeManager(accounts.other.address, "Beta Manager"),
      )
  ...
  describe("members", () => {
    ...
    it("bulk removeMembers removes existing; non-members are ignored", async () => {
      ...
      await managed.connect(accounts.manager).addMembers([
        accounts.other.address,
        accounts.third.address,
    ...
    it("swap-and-pop keeps structure: removing middle compacts array", async () => {
      ...
      // Now internal array is [A, B, C] (order not guaranteed externally, we use membership checks)
  ...
  describe("trader", () => {
    it("only manager can set/remove trader; zero address rejected", async () => {
      const { managed, accounts } = await loadFixture(setupFixture);

      await expect(
        managed.connect(accounts.manager).setTrader(ethers.ZeroAddress),
      ).to.be.revertedWithCustomError(managed, "InvalidTrader");

...
describe("SlippageAccumulator", () => {
  ...
  beforeEach(async () => {
    ...
    const MockSupportedAsset = await ethers.getContractFactory(
      "MockSupportedAsset"
    );
    ...
    const MockERC20Extended = await ethers.getContractFactory(
      "MockERC20Extended"
    );
    ...
    const SlippageAccumulator = await ethers.getContractFactory(
      "SlippageAccumulator"
    );
  ...
  describe("constructor", () => {
    it("reverts with null poolFactory", async () => {
      ...
      await expect(
        SlippageAccumulator.connect(owner).deploy(
          ethers.ZeroAddress,
          DECAY_TIME,
          MAX_CUMULATIVE_SLIPPAGE
        )
  ...
  describe("assetValue", () => {
    it("computes correct USD value", async () => {
      ...
      const value = await slippageAccumulator.assetValue(
        srcTokenAddress,
        amount
      );
  ...
  describe("updateSlippageImpact", () => {
    it("does nothing if srcAsset is not supported", async () => {
      ...
      const managerData = await slippageAccumulator.managerData(
        poolManagerAddress
      );
    ...
    it("updates accumulated slippage and timestamp when dstValue < srcValue", async () => {
      ...
      const cumulative =
        await slippageAccumulator.getCumulativeSlippageImpact(
          poolManagerAddress
        );
    ...
    it("reverts when new cumulative slippage exceeds the maximum", async () => {
      ...
      await slippageAccumulator
        .connect(owner)
        .setMaxCumulativeSlippage(tinyMax);
  ...
  describe("owner functions", () => {
    it("setDecayTime can only be called by owner and emits event", async () => {
      ...
      await expect(
        slippageAccumulator.connect(other).setDecayTime(newDecay)
      )
      ...
      await expect(
        slippageAccumulator.connect(owner).setDecayTime(newDecay)
      )
    ...
    it("setMaxCumulativeSlippage can only be called by owner and emits event", async () => {
      ...
      await expect(
        slippageAccumulator.connect(other).setMaxCumulativeSlippage(newMax)
      )
        .to.be.revertedWithCustomError(
          slippageAccumulator,
          "OwnableUnauthorizedAccount"
      ...
      await expect(
        slippageAccumulator
          .connect(owner)
          .setMaxCumulativeSlippage(newMax)
      )

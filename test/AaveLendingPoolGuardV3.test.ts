...
describe("AaveLendingPoolGuardV3", () => {
  ...
  function encodeBorrow(
    ...
    return aaveIface.encodeFunctionData("borrow", [
      asset,
      amount,
      rateMode,
      0,
  ...
  it("reverts when data provider = zero", async () => {
    const G = await ethers.getContractFactory("AaveLendingPoolGuardV3");
    await expect(G.deploy(ZeroAddress)).to.be.revertedWith(
      "Frgmnt: zero V3 provider"
    );
  });
  ...
  it("supply fails: asset not lending-enabled", async () => {
    ...
    await expect(
      guard.txGuard(poolManager.target, lendingPool, data)
  ...
  it("setUserUseReserveAsCollateral → txType=11", async () => {
    const data = aaveIface.encodeFunctionData(
  ...
  it("swapBorrowRateMode → txType=14 (stable → variable)", async () => {
    const data = aaveIface.encodeFunctionData("swapBorrowRateMode", [
      assetLending,
      1,
    ]);
  ...
  it("rebalance fails: unsupported asset", async () => {
    const data = aaveIface.encodeFunctionData(
      "rebalanceStableBorrowRate",
      [unsupportedAsset, poolLogicAddr]
    );

...
import type { ERC20Guard, MockERC20, MockGuardInfo, MockPoolManagerLogic } from "../typechain-types"
...
describe("ERC20Guard", () => {
	...
	it("emits Approve and returns txType=1 for valid approve calls", async () => {
		const { guard, poolManager, guardInfo, poolLogic, manager, spender, otherGuard } = await deployMocks()
	...
	it("removeAssetCheck reverts when pool has non-zero balance", async () => {
		...
		await expect(guard.removeAssetCheck(pool, await token.getAddress())).to.be.revertedWithCustomError(
			guard,
			"NonZeroAssetBalance"
		)
	})

...
describe("ERC20Guard", () => {
  async function deployMocks() {
    const [deployer, manager, poolLogic, spender, otherGuard] =
  ...
  it("emits Approve and returns txType=1 for valid approve calls", async () => {
    ...
    await expect(
      guard.txGuard(await poolManager.getAddress(), ethers.ZeroAddress, data)
    )
  ...
  it("withdrawProcessing returns correct pro-rata amount and empty txs", async () => {
    ...
    const [withdrawAsset, withdrawAmount, txs] =
      await guard.withdrawProcessing.staticCall(
        pool,
  ...
  it("getBalance returns ERC20 balance of pool", async () => {
    ...
    expect(
      await guard.getBalance(pool, await token.getAddress())
    ).to.equal(0n);

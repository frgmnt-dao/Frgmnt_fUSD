...
describe("TxDataUtils", () => {
	...
	const ifaceFoo = new ethers.Interface(["function foo(address a,uint256 b,bytes data,uint256[] arr)"])
	...
	it("getMethod returns correct selector and reverts on short data", async () => {
		...
		const calldata = ifaceFoo.encodeFunctionData("foo", [addr, 123n, "0x12345678abcd", [1n, 2n, 3n]])

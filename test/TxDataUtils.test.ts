...
describe("TxDataUtils", () => {
  ...
  it("getMethod returns correct selector and reverts on short data", async () => {
    ...
    await expect(utils.getMethod("0x1234")).to.be.revertedWith(
  ...
  it("getParams returns calldata without selector and handles empty params", async () => {
    ...
    await expect(utils.getParams("0x")).to.be.revertedWith(
      "TxDataUtils: no selector"
    );
  });
  ...
  it("getBytes decodes dynamic bytes and handles invalid offset and bounds", async () => {
    ...
    // invalid offset (>=20)
    await expect(utils.getBytes(calldata, 2, 20)).to.be.revertedWith(
      "invalid offset"
    );
    ...
    const relativeOffset = BigInt(
      "0x" + Buffer.from(pointerWord).toString("hex")
    );
  ...
  it("getArrayLength/getArrayLast/getArrayIndex work and revert correctly", async () => {
    ...
    await expect(utils.getArrayIndex(calldata, 0, 3)).to.be.revertedWith(
    ...
    await expect(utils.getArrayLast(calldataEmpty, 0)).to.be.revertedWith(
      "input is not array"
    );
    await expect(
      utils.getArrayIndex(calldataEmpty, 0, 0)
    ).to.be.revertedWith("input is not array");
  });
  ...
  it("read4left and read32 behave correctly and enforce bounds", async () => {
    ...
    // out-of-bounds
    await expect(utils.read4left("0x1234", 1)).to.be.revertedWith(
      "Reading bytes out of bounds"
    );
    ...
    // invalid length > 32
    await expect(utils.read32(data, 0, 33)).to.be.revertedWith(
      "invalid length"
    );

    // out-of-bounds length
    await expect(utils.read32("0x1234", 0, 4)).to.be.revertedWith(
      "Reading bytes out of bounds"
    );
  });
  ...
  it("sliceUint reads uint256 from bytes and reverts on out of range (indirectly)", async () => {
    ...
    await expect(utils.read32(data, 1, 32)).to.be.revertedWith(
      "Reading bytes out of bounds"
    );

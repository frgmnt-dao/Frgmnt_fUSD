...
describe("AssetHandler", () => {
  ...
  it("initializes correctly with assets and sets owner + timeout", async () => {
    ...
    expect(await handler.priceAggregators(asset)).to.equal(
  ...
  describe("setChainlinkTimeout", () => {
    ...
    it("reverts when called by non-owner", async () => {
      ...
      await expect(
        handler.connect(other).setChainlinkTimeout(newTimeout)
      )
  ...
  describe("addAsset", () => {
    it("adds asset correctly and emits event", async () => {
      ...
      await expect(
        handler.addAsset(asset, assetType, await mock.getAddress())
      )
    ...
    it("reverts if aggregator is zero address", async () => {
      ...
      await expect(
        handler.addAsset(asset, 1, ethers.ZeroAddress)
      ).to.be.revertedWith("Frgmnt: aggregator=0");
    });
    ...
    it("reverts when non-owner calls addAsset", async () => {
      ...
      await expect(
        handler.connect(other).addAsset(asset, 1, await mock.getAddress())
      )
        .to.be.revertedWithCustomError(handler, "OwnableUnauthorizedAccount")
        .withArgs(other.address);
    });
  ...
  describe("addAssets", () => {
    ...
    it("adds multiple assets correctly", async () => {
      ...
      expect(await handler.priceAggregators(asset1)).to.equal(
        await mock1.getAddress()
      );
      expect(await handler.priceAggregators(asset2)).to.equal(
  ...
  describe("removeAsset", () => {
    it("removes asset mappings and emits event", async () => {
      ...
      await expect(handler.removeAsset(asset))
        .to.emit(handler, "RemovedAsset")
        .withArgs(asset);

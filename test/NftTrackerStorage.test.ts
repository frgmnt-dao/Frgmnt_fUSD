...
describe("NftTrackerStorage", function () {
  ...
  const makeType = (label: string) =>
    ethers.keccak256(ethers.toUtf8Bytes(label));

  beforeEach(async function () {
    [deployer, guard, other, anotherGuard, pool, pool2] =
    ...
    await mockGuardInfo.setContractGuard(
      anotherGuardedContract,
      anotherGuard.address
    );

    // Deploy NftTrackerStorage
    const NftTrackerStorage = await ethers.getContractFactory(
      "NftTrackerStorage"
    );
  ...
  it("cannot be initialized twice", async function () {
    await expect(
      nftTracker.initialize(mockGuardInfo.target)
    ).to.be.reverted; // OZ Initializable revert
  });
  ...
  it("allows correct guard to call guarded functions", async function () {
    ...
    await expect(
      nftTracker
        .connect(guard)
        .addData(guardedContract, nftType, pool.address, data)
    ).to.not.be.reverted;
  ...
  it("uses different guards for different guarded contracts", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addData(guardedContract, nftTypeA, pool.address, dataA);
  ...
  it("stores and retrieves generic NFT data correctly", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addData(guardedContract, nftType, pool.address, data1);
    await nftTracker
      .connect(guard)
      .addData(guardedContract, nftType, pool.address, data2);
  ...
  it("separates data per NFT type and pool", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addData(guardedContract, typeA, pool.address, dataA1);
    await nftTracker
      .connect(guard)
      .addData(guardedContract, typeB, pool.address, dataB1);

    // Another pool with same "typeA"
    await nftTracker
      .connect(guard)
      .addData(guardedContract, typeC, pool2.address, dataC1);
  ...
  it("removes data by index using swap-and-pop", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addData(guardedContract, nftType, pool.address, data3);

    // Remove middle element index 1 (data2)
    await nftTracker
      .connect(guard)
      .removeData(guardedContract, nftType, pool.address, 1);
  ...
  it("reverts removeData with invalid index", async function () {
    ...
    await expect(
      nftTracker
        .connect(guard)
        .removeData(guardedContract, nftType, pool.address, 5)
  ...
  it("adds uint256 IDs and decodes them correctly", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addUintId(guardedContract, nftType, pool.address, id1, 10);
    await nftTracker
      .connect(guard)
      .addUintId(guardedContract, nftType, pool.address, id2, 10);
  ...
  it("respects maxPositions when adding uint IDs", async function () {
    ...
    await nftTracker
      .connect(guard)
      .addUintId(guardedContract, nftType, pool.address, id1, 1);

    await expect(
      nftTracker
        .connect(guard)
        .addUintId(guardedContract, nftType, pool.address, id2, 1)
  ...
  it("removes uint256 IDs via removeUintId (guard)", async function () {
    ...
    await nftTracker
      .connect(guard)
      .removeUintId(guardedContract, nftType, pool.address, id1);
  ...
  it("reverts removeUintId when ID not found", async function () {
    ...
    await expect(
      nftTracker
        .connect(guard)
        .removeUintId(guardedContract, nftType, pool.address, 999n)
  ...
  it("allows owner to remove data by uint ID", async function () {
    ...
    await nftTracker
      .connect(deployer)
      .removeDataByUintId(nftType, pool.address, id1);
  ...
  it("reverts owner removeDataByUintId when ID not found", async function () {
    ...
    await expect(
      nftTracker
        .connect(deployer)
        .removeDataByUintId(nftType, pool.address, 999n)
    ).to.be.revertedWith("not found");
  });
  ...
  it("allows owner to removeDataByIndex", async function () {
    ...
    await nftTracker
      .connect(deployer)
      .removeDataByIndex(nftType, pool.address, 0);
  ...
  it("allows owner to addDataByUintId", async function () {
    ...
    await nftTracker
      .connect(deployer)
      .addDataByUintId(nftType, pool.address, id1);
  ...
  it("prevents non-owner from using owner-only functions", async function () {
    ...
    await expect(
      nftTracker
        .connect(other)
        .addDataByUintId(nftType, pool.address, 1234n)
    )
      .to.be.revertedWithCustomError(nftTracker, "OwnableUnauthorizedAccount")
      .withArgs(other.address);
  });

...
describe("NftTrackerStorage", function () {
	...
	it("allows correct guard to call guarded functions", async function () {
		...
		await expect(nftTracker.connect(guard).addData(guardedContract, nftType, pool.address, data)).to.not.be.reverted
	...
	it("uses different guards for different guarded contracts", async function () {
		...
		await nftTracker.connect(anotherGuard).addData(anotherGuardedContract, nftTypeB, pool2.address, dataB)
	...
	it("reverts owner removeDataByUintId when ID not found", async function () {
		...
		await expect(nftTracker.connect(deployer).removeDataByUintId(nftType, pool.address, 999n)).to.be.revertedWith(
			"not found"
		)
	})
